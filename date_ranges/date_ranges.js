...
function printDate (days, months, years, difference) {
  var startOrdinal = getOrdinal(days[0]);
  var endOrdinal = getOrdinal(days[1]);

  if (difference < 365) {
    if (years[0] === years[1] && Number(years[0]) === new Date().getFullYear()) {
      console.log ([monthsArray[months[0] - 1] + ' ' + days[0] + startOrdinal, days[1] + endOrdinal]);
    }
    else {
      console.log ([monthsArray[months[0] - 1] + ' ' + days[0] + startOrdinal + ', ' + years[0], days[1] + endOrdinal]);
    }
  }
  else {
...
function makeFriendlyDates (dateArray) {
  if (dateArray[0][8] === '0') {
    var days = [dateArray[0].slice(9, 10), dateArray[1].slice(9, 10)];
  }
  else {
    var days = [dateArray[0].slice(8, 10), dateArray[1].slice(8, 10)];
  }
  ...
  printDate(days, months, years, difference);
}
